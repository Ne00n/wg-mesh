...
class Templator:
    ...
    def genServer(self,interface,config,payload,serverIP,serverPort,wgobfsSharedKey=""):
        ...
        if linkType == "ipt_xor": wgobfs += f"sudo iptables -t mangle -A OUTPUT -p udp --dport {serverPort} -j XOR --keys "{wgobfsSharedKey}";\n"
        if linkType == "ipt_xor": wgobfs += f"sudo iptables -t mangle -A INPUT -p udp --sport {serverPort} -j XOR --keys "{wgobfsSharedKey}";\n"
