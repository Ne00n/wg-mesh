...
class Wireguard(Base):
    ...
    def updateConfig(self):
        ...
        if not "notifications" in self.config: self.config['notifications'] = {"enabled":False,"server":""}
    ...
    def init(self,id,listen):
        ...
        "bird":{"ospfv2":True,"ospfv3":True,"area":0,"tick":1,"client":False},"notifications":{"enabled":False,"server":""}}
    ...
    def filesToLinks(self,files):
        ...
        for findex, filename in enumerate(files):
            ...
            if "endpoint" in config:
                ...
                if "Client" in config:
                    remotePublic = re.findall(f"Client\s([0-9a-z.:]+)",config,re.MULTILINE)[0]
            #get remote endpoint
