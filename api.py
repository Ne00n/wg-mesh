...
tokens = {"connect":[],"peer":[]}
for i in range(3):
    token = "TOK" + ''.join(secrets.choice(string.ascii_uppercase + string.digits) for _ in range(32))
...
def index():
    ...
    return HTTPResponse(status=200, body={'connectivity':connectivity,'geo':geo,'linkTypes':config['linkTypes'],'subnetPrefix':subnetPrefix})
